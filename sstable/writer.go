...
func shouldFlushWithHints(
	...
	if len(sizeClassHints) == 0 {
		return shouldFlushWithoutHints(
			key, valueLen, restartInterval, estimatedBlockSize, numEntries, targetBlockSize, sizeThreshold)
	}
	...
	newEstimatedSize := estimatedBlockSize + key.Size() + valueLen + 18
...
// space wasted by an allocation.
func blockSizeClass(blockSize int, sizeClassHints []int) (int, bool) {
	for _, sizeClass := range sizeClassHints {
		if sizeClass >= blockSize {
			return sizeClass, true
		}
	}
	return -1, false
}
